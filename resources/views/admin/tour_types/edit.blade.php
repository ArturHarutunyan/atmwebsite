...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <h2>
                {{trans('admin.edit_the_tour_type')}}
            </h2>
            <form action="{{route('tour_type.update',['id'=>$tour_type->id])}}" method="post">
                {{ csrf_field() }}
                ...
                <div class="tab-content">
                    @foreach(array_keys(config('translatable.locales')) as $lang)
                        @if($tour_type->hasTranslation($lang))
                            <div id="{{$lang}}" class="tab-pane fade @if(App::isLocale($lang))in active @endif">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name_{{$lang}}" id="name_{{$lang}}" value="{{$tour_type->getTranslation($lang)->name}}" class="form-control">
                                </div>
                            </div>
                        @endif
                    @endforeach
