...
@section('content')
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <h2>
                {{trans('admin.edit_the_trustee')}}
            </h2>
            <form action="{{route('trustee.update',['id'=>$trustee->id])}}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                    ...
                    @foreach(array_keys(config('translatable.locales')) as $lang)
                        <div id="{{$lang}}" class="tab-pane fade @if(App::isLocale($lang))in active @endif">
                            <div class="form-group">
                                ...
                                <input type="text" name="title_{{$lang}}" id="title_{{$lang}}" value="{{$trustee->getTranslation($lang)->title}}" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="text_content_{{$lang}}">{{Lang::get('admin.text_content',[],$lang)}}</label>
                                <textarea id="text_content_{{$lang}}" class="ckeditor-content" name="text_content_{{$lang}}" class="form-control">{{$trustee->getTranslation($lang)->text_content}}</textarea>
                            </div>
                        </div>
                    @endforeach
                </div>
                <div class="form-group">
                    ...
                    @if($trustee->image)
                        <img class="img-responsive old-image" src="{{asset($trustee->image)}}">
                    @endif
