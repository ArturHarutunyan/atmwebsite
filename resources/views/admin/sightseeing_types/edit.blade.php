...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <h2>
                {{trans('admin.edit_the_sightseeing_type')}}
            </h2>
            <form action="{{route('sightseeing_type.update',['id'=>$sightseeing_type->id])}}" method="post">
                {{ csrf_field() }}
                    ...
                    @foreach(array_keys(config('translatable.locales')) as $lang)
                        <div id="{{$lang}}" class="tab-pane fade @if(App::isLocale($lang))in active @endif">
                            <div class="form-group">
                                ...
                                <input type="text" name="name_{{$lang}}" id="name_{{$lang}}" value="{{$sightseeing_type->getTranslation($lang)->name}}" class="form-control">
                            </div>
                        </div>
                    @endforeach
                </div>
