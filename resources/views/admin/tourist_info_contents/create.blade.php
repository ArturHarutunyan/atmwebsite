...
@section('content')
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <h2>
                {{trans('admin.add_new_tourist_info_content')}}
            </h2>
            <form action="{{route('tourist_info_content.store')}}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                    ...
                    @foreach(array_keys(config('translatable.locales')) as $lang)
                        <div id="{{$lang}}" class="tab-pane fade @if(App::isLocale($lang))in active @endif">
                            <div class="form-group">
                                ...
                                <textarea id="visa_content_{{$lang}}" class="ckeditor-content form-control" name="visa_content_{{$lang}}"></textarea>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="currency_content_{{$lang}}" class="ckeditor-content form-control" name="currency_content_{{$lang}}"></textarea>
                            </div>
                            <div class="form-group">
                                <label for="safety_content_{{$lang}}">{{Lang::get('admin.safety_content',[],$lang)}}</label>
                                <textarea id="safety_content_{{$lang}}" class="ckeditor-content form-control" name="safety_content_{{$lang}}"></textarea>
                            </div>
                        </div>
                    @endforeach
                </div>
