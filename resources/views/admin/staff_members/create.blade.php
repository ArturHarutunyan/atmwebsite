...
@section('content')
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <h2>
                {{trans('admin.add_a_new_staff_member')}}
            </h2>
            <form action="{{route('staff_member.store')}}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                    ...
                    @foreach(array_keys(config('translatable.locales')) as $lang)
                        <div id="{{$lang}}" class="tab-pane fade @if(App::isLocale($lang))in active @endif">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="surname_{{$lang}}" id="surname_{{$lang}}" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="position_{{$lang}}">{{Lang::get('admin.position',[],$lang)}}</label>
                                <input type="text" name="position_{{$lang}}" id="position_{{$lang}}" class="form-control">
                            </div>
                        </div>
                    @endforeach
                </div>
