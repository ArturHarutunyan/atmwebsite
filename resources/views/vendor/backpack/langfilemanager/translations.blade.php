...
{{--				@foreach(config('translatable.locales') as $key=>$value)
					<a href="{{route('admin.lang_swish',['language'=>$key])}}" class="btn btn-primary ladda-button">{{$value}}</a>
				@endforeach--}}
				@foreach ($languages as $lang)
{{--
				<option value="http://10.10.10.254/{{"en/admin/language/texts/{$lang->abbr}" }}" {{ $currentLang == $lang->abbr ? 'selected' : ''}}>{{ $lang->name }}</option>
--}}
				<option value="{{"{$lang->abbr}" }}" {{ $currentLang == $lang->abbr ? 'selected' : ''}}>{{ $lang->name }}</option>
				@endforeach
			</select>
		</small>
	  </h3>
    ...
    <div class="box-body">
		...
		@if (!empty($fileArray))
			<form
				...
				action="{{route('backpack.store_languages',['lang'=>$currentLang, 'file'=>$currentFile])}}"
{{--
		  		action="{{ nguage/texts/{$currentLang}/{$currentFile}" }}"
--}}
		  		>
				{!! csrf_field() !!}
