...
Selector.prototype.renderSelected = function (selectedElement) {
    ...
    selectedText.innerHTML = selectedElement.innerHTML;
...
Selector.prototype.renderOptions = function () {
    ...
    for (let i = 0; i < this.currentElement.childElementCount; ++i) {
        ...
        let currentOption = this.currentElement[i]
        let optionAdditionalPostText = currentOption.getAttribute('data-AdditionalPostText');

        let prefix_icon_classes  = currentOption.getAttribute('data-prefix_icon')

        let prefix_icon 
        if(prefix_icon_classes && prefix_icon_classes.trim()){
            prefix_icon = `<i class='${prefix_icon_classes}'></i>`;
        }
        ...
        let text = this.currentElement[i].innerHTML;





        // icon_container_fostFix
        let icon_pretFix = prefix_icon;
        let icon_postFix = this.currentElement.closest('.select_container').querySelector('.icon_container_postFix') && this.currentElement.closest('.select_container').querySelector('.icon_container_postFix').innerHTML;
        
        if (icon_pretFix) {
            text = icon_pretFix + text;
        }
        if (optionAdditionalPostText) text += `<span class='option_inside_postText'> ${optionAdditionalPostText} </span>`;

        if (icon_postFix) text += icon_postFix
    ...
    for (let event in eventList) {
        ...
        searchBox.addEventListener(event, function () {
            ...
            for (let i = 0; i < options.length; ++i) {
                ...
                let value = option.innerHTML.toLowerCase().trim();
...
Selector.prototype.createEvents = function () {
    ...
    instanceSelected.onclick = function () {
        ...
        if(!currentInstance.classList.contains('open')){
            this.closest('.select_container').querySelector('select').dispatchEvent(new Event('change'))
        }
        if (isSearch) {
...
Selector.prototype.changeSelectedOption = function (clicked) {
    ...
    let target = clicked.target.closest('.selector-option') || clicked.target;

    // console.log(target.parentNode.parentNode, target.closest('.selector-element'));
    ...
    let instanceParent = target.closest('.selector-element');
    ...
    selected.querySelector('p').innerHTML = text;
