...
{
    public function switchLang(Request $request, $lang, $hash='')
    {
        ...
        if (array_key_exists($lang, config('translatable.locales'))) {
            ...
            if ($route_name && Lang::has('routes.'.$route_name, $lang)) {
                ...
                if (count($query)) {
                    return redirect()->to($lang . '/' .  trans('routes.' . $route_name, [], $lang) . '?' . http_build_query($query).$hash);
                }
                return redirect()->to($lang . '/' .  trans('routes.' . $route_name, [], $lang).$hash);
            }
            ...
            if (count($query)) {
                ...
                return redirect()->to(implode('/', $segments).'?'.http_build_query($query).$hash);
            }

            return redirect()->to(implode('/', $segments).$hash);
        }
        return redirect()->back();
    }
    public function switchLangAdmin(Request $request, $lang)
    {
        // Store the URL on which the user was
        $previous_url = url()->previous();
        // Transform it into a correct request instance
        ...
        if (array_key_exists($lang, config('translatable.locales'))) {
            ...
            if ($route_name && Lang::has('routes.'.$route_name, $lang)) {
                // Translate the route name to get the correct URI in the required language, and redirect to that URL.
                if (count($query)) {
                    return redirect()->to($lang . '/admin/' .  trans('routes.' . $route_name, [], $lang) . '?' . http_build_query($query));
                }
                return redirect()->to($lang . '/admin/' .  trans('routes.' . $route_name, [], $lang));
            }
            // Replace the first segment by the new language code
            $segments[0] = $lang;
            // Redirect to the required URL
            if (count($query)) {
                return redirect()->to(implode('/admin/', $segments).'?'.http_build_query($query));
            }
            return redirect()->to(implode('/admin/', $segments));
        }
        return redirect()->back();
    }
}